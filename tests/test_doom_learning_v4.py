...
import numpy as np
import pytest
from doom_learning_v4.brain import MemoryBrain
...
def fixture_brain(tmp_path, eta=.001, modulation=True):
    ...
    return MemoryBrain(p,eta=eta,circuit=c,modulation_mask=c["dan_index"]>=0,kc_rest=-52.,adaptation_jump=0.)
...
def test_checkpoint_rejects_different_original_weights(tmp_path):
    ...
    c=MemoryBrain(graph,circuit=b.circuit,modulation_mask=b.modulation_mask,kc_rest=-52.,adaptation_jump=0.)
    with pytest.raises(ValueError,match='provenance'):c.restore(p)


def test_spike_adaptation_is_cell_specific_and_checkpointed(tmp_path):
    b=fixture_brain(tmp_path);b.adaptation_jump=8.
    first,_=b.step([],50,stimulation=([0,4],12),lamina_bias=0)
    late,_=b.step([],950,stimulation=([0,4],12),lamina_bias=0)
    assert first[0]/50>late[0]/950 and b.adaptation[0]>0
    assert b.adaptation[4]==0 and late[4]>late[0]
    p=tmp_path/'adaptive.npz';b.checkpoint(p)
    c,_=b.step([],50,stimulation=([0],12),lamina_bias=0);state=b.adaptation.copy()
    b.restore(p);d,_=b.step([],50,stimulation=([0],12),lamina_bias=0)
    np.testing.assert_array_equal(c,d);np.testing.assert_array_equal(state,b.adaptation)


def test_adaptive_subthreshold_solution_matches_closed_form(tmp_path):
    b=fixture_brain(tmp_path)
    b.adaptation[0]=8.;b.v[0]=-52.;b.last[0]=-1
    b.step([],50,lamina_bias=0)
    a=np.exp(-50/20);c=np.exp(-50/200)
    assert b.v[0]==pytest.approx(-52-8*200/(200-20)*(c-a),abs=.002)
    assert b.adaptation[0]==pytest.approx(8*c,abs=.002)
