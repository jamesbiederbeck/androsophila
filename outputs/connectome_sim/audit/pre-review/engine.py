...
def advance(ptr,post,weight,v,g,refractory,drive,queue,queue_count,cursor,steps,dt,counts,active,active_flag,nactive):
    ...
    for step in range(steps):
        # Delivery occurs after integration/threshold/reset, matching the
        ...
        for q in range(queue_count[future]):
            i=queue[future,q];v[i]=-52;g[i]=0;refractory[i]=int(round(2.2/dt))-1
        cursor+=1
    return cursor
...
class Brain:
    ...
    def step(self,luminance,duration_ms,sugar=False,lamina_bias=12.0):
        ...
        # 10 ms receptor adaptation time is a fixed experimental assumption.
