...
extern "C" void neural_advance(
 ...
 const int delay=std::lround(1.8f/dt),rfc=std::lround(2.2f/dt)-1,slots=delay+1;
 ...
 auto evolve=[&](int i,int64_t now,float current){
   ...
   const int skip=(int)(d<refractory[i]?d:refractory[i]);refractory[i]-=skip;d-=skip;
 ...
 for(int t=0;t<steps;t++,(*clock)++){
   ...
   for(int k=0;k<original;k++){
     ...
     if(v[i]>-45.f){queue[future*n+queue_count[future]++]=i;counts[i]++;}
   ...
   *nactive=kept;
   for(int q=0;q<queue_count[slot];q++){
     ...
     for(int64_t e=ptr[i];e<ptr[i+1];e++){
       const int j=post[e];evolve(j,*clock,drive[j]);g[j]+=weight[e];awaken(j);
     }
   }
   queue_count[slot]=0;
