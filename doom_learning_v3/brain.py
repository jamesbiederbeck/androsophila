...
LIBRARY=(OUT/'physiology-v3')/('libmemory.dylib' if sys.platform=='darwin' else 'libmemory.so')
MODEL='kc-rest-minus60-v3'
...
class MemoryBrain(NativeBrain):
    def __init__(self,path=GRAPH,*,eta=.001,circuit=None,modulation_mask=None,kc_rest=-60.):
        super().__init__(path)
        ...
        self.advance.argtypes=[C.c_int]+[C.c_void_p]*11+[C.c_int,C.c_float]+[C.c_void_p]*5+[
            ...
            C.c_float,C.c_float,C.c_float,C.c_int,C.c_void_p,C.c_void_p,C.c_void_p,C.c_void_p]
