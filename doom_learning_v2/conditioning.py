...
def run(out=OUT/'physiology-v2/conditioning.json',current=7.2):
    import numpy as np
...
if __name__=='__main__':
    require_single_blas_thread();p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',default=str(OUT/'physiology-v2/conditioning.json'))
    p.add_argument('--current',type=float,default=7.2);args=p.parse_args()
