...
def run(out=OUT/'physiology-v4/conditioning.json',current=15.2):
    import numpy as np
    ...
    def probe(cue):
        ...
        c,_=b.step(dark,1000,learning=False,stimulation=(cue,current),lamina_bias=0)
    ...
    for condition in ['paired','backward','frozen','no_imposed_US','reset_memory']:
        ...
        for begin,end in zip(bounds,bounds[1:]):
            stimulation=[]
            if cs_start<=begin<cs_end:stimulation.append((cues[0],current))
    ...
    report={'schema':1,'assay':'imposed-KC-activity, full graph','circuit':b.circuit['report'],'parameters':PARAMETERS,
        ...
        'stimulus':{'KC_current_mv_equivalent':current,'KC_cells_per_cue':64,'DAN_current_mv_equivalent':200,
...
if __name__=='__main__':
    require_single_blas_thread();p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',default=str(OUT/'physiology-v4/conditioning.json'))
    p.add_argument('--current',type=float,default=15.2);args=p.parse_args()
    run(args.out,args.current)
