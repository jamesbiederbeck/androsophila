...
LIBRARY=(OUT/'physiology-v4')/('libmemory.dylib' if sys.platform=='darwin' else 'libmemory.so')
MODEL='kc-adaptive-lif-v4'
...
class MemoryBrain(NativeBrain):
    def __init__(self,path=GRAPH,*,eta=.001,circuit=None,modulation_mask=None,kc_rest=-60.,adaptation_jump=8.,adaptation_tau=200.):
        super().__init__(path)
    ...
    def configuration_signature(self):
        ...
        return {'initial_weight':self.initial_weight_sha256,'modulation_mask':digest(self.modulation_mask),'rest':digest(self.rest),
                'adaptation_jump':self.adaptation_jump,'adaptation_tau':self.adaptation_tau,
