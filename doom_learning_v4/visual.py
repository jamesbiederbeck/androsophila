...
def projection(brain,a):
    ...
    mapped=[];hexes=[];confidence=[]
    for i in known:
        edges=np.arange(brain.ptr[i],brain.ptr[i+1]);targets=brain.post[edges]
        valid=a.assignedOlHex1.iloc[targets].notna().to_numpy() & a.assignedOlHex2.iloc[targets].notna().to_numpy()
        votes={}
        ...
        h=max(votes,key=votes.get);mapped.append(i);hexes.append(h);confidence.append(votes[h]/sum(votes.values()))
    mapped=np.asarray(mapped,dtype=np.int32);hexes=np.asarray(hexes)
    ...
    for side in ['L','R']:
        ...
        lo=oldxy[original].min(axis=0);span=np.ptp(oldxy[original],axis=0)
        z=(xy[select]-lo)/span
        uv[select,0]=.6*z[:,0] if side=='L' else .4+.6*(1-z[:,0])
        uv[select,1]=1-z[:,1]
    if not a.rootSide.iloc[mapped].isin(['L','R']).all():raise ValueError('Unresolved eye side')
    return mapped,np.clip(uv,0,1).astype(np.float32),np.asarray(confidence,dtype=np.float32)
...
class VisualMemoryBrain(MemoryBrain):
    def __init__(self,**kwargs):
        ...
        for i in np.flatnonzero(a.type.fillna('').str.startswith('R8')):
            edges=np.arange(self.ptr[i],self.ptr[i+1]);e=edges[a.type.iloc[self.post[edges]].eq('aMe12').to_numpy()]
        ...
        self.visual_report={'model':'r8-rgb-ame12-v1','mapped_R8p':int((self.r8_channel==2).sum()),
            ...
            'corrected_existing_edges':len(corrected),'corrected_edge_sha256':digest(self.corrected_edges),
            ...
            'evidence':['https://doi.org/10.1038/s41586-023-06681-6','https://doi.org/10.1038/s41467-024-49616-z','https://doi.org/10.7554/eLife.71858'],
            'validated':False}
