...
LIBRARY=(OUT/'physiology-v5')/('libmemory.dylib' if sys.platform=='darwin' else 'libmemory.so')
MODEL='centered-antihebbian-v5'
from .rule import PARAMETERS as RULE_PARAMETERS
PARAMETERS={**RULE_PARAMETERS,'neural_dt_ms':.1,'modulator_delivery_trace_ms':100.,
    'interpretation':'Baseline-centered anti-Hebbian rate-rule adaptation on the full graph. Tonic currents calibrated to published baseline rates; associative gain and trace constants remain hypotheses. No game-based parameter fitting.'}
...
class MemoryBrain(NativeBrain):
    def __init__(self,path=GRAPH,*,eta=.001,circuit=None,modulation_mask=None,tonic=None,dan_baseline_hz=None):
        super().__init__(path)
        ...
        self.advance.argtypes=[C.c_int]+[C.c_void_p]*11+[C.c_int,C.c_float]+[C.c_void_p]*5+[
            ...
            C.c_float,C.c_float,C.c_float,C.c_int,C.c_void_p,C.c_void_p,C.c_void_p]
        ...
        if modulation_mask is None:
            ...
            modulation_mask=neurons.neurotransmitter.isin(['dopamine','octopamine','serotonin']).to_numpy(dtype=np.uint8)
        self.modulation_mask=np.asarray(modulation_mask,dtype=np.uint8).copy()
        if self.modulation_mask.shape!=(self.n,) or np.any(self.modulation_mask>1):raise ValueError('Invalid modulation mask')
        self.eta=float(eta)
        ...
        self.memory_u=np.zeros_like(self.rate_kc);self.memory_w=np.zeros_like(self.rate_kc)
        self.tonic=np.zeros(self.n,dtype=np.float32) if tonic is None else np.asarray(tonic,dtype=np.float32).copy()
        self.dan_baseline_hz=np.zeros(len(self.circuit['dan']),dtype=np.float64) if dan_baseline_hz is None else np.asarray(dan_baseline_hz,dtype=np.float64).copy()
    ...
    def _neural_step(self,luminance,duration_ms,*,learning=False,stimulation=None,lamina_bias=12.):
        ...
        self.advance(self.n,*[x.ctypes.data for x in arrays],steps,self.dt,
            *[getattr(self,k).ctypes.data for k in ['counts','active','active_flag','nactive','last']],
            ...
            self.modulation.ctypes.data,self.modulation_last.ctypes.data,self.modulation_mask.ctypes.data)
